...
From the survey data of Groundcover Plants, we can analyze the year wise composition of forest by native/non-native and invasive. 
...
gc_plants_1937 <- subset(gc_plants, gc_plants$Year==1937)
gc_plants_2002 <- subset(gc_plants, gc_plants$Year==2002)
gc_plants_2006 <- subset(gc_plants, gc_plants$Year==2006)
gc_plants_2011 <- subset(gc_plants, gc_plants$Year==2011)
gc_plants_2016 <- subset(gc_plants, gc_plants$Year==2016)

# Create vectors for all the years and species -> native, not-native and invasive
year <- c(1937, 2002, 2006, 2011, 2016)
native <- c(length(which(gc_plants_1937$Native=='y')), length(which(gc_plants_2002$Native=='y')), length(which(gc_plants_2006$Native=='y')), length(which(gc_plants_2011$Native=='y')), length(which(gc_plants_2016$Native=='y')))
non_native <- c(length(which(gc_plants_1937$Non.native=='y')), length(which(gc_plants_2002$Non.native=='y')), length(which(gc_plants_2006$Non.native=='y')), length(which(gc_plants_2011$Non.native=='y')), length(which(gc_plants_2016$Non.native=='y')))
invasive <- c(length(which(gc_plants_1937$Invasive=='y')), length(which(gc_plants_2002$Invasive=='y')), length(which(gc_plants_2006$Invasive=='y')), length(which(gc_plants_2011$Invasive=='y')), length(which(gc_plants_2016$Invasive=='y')))

#Create dataframe with the vectors
gc_plants_trend <- data.frame(year, native, non_native, invasive, stringsAsFactors = FALSE)

gc_plants_trendL <- melt(gc_plants_trend, id.vars = c("year"))
colnames(gc_plants_trendL)[2] <- "type"
colnames(gc_plants_trendL)[3] <- "count"

gc_plants_trendL[year != 1937,] %>%
  ggplot(aes(x = year, y = count, color = type)) + 
  geom_line(size=1) + geom_point(size=2) + 
  ggtitle("Woody Plants Count from 2002 to 2016") + labs(x="Years", y="Count") +
  theme_bw()
